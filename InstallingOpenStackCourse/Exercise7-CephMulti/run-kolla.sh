...
cp /vagrant/.vagrant/machines/controller2/virtualbox/private_key .ssh/controller2.pem
cp /vagrant/.vagrant/machines/controller3/virtualbox/private_key .ssh/controller3.pem
...
chmod 600 .ssh/controller2.pem
chmod 600 .ssh/controller3.pem
...
ssh -i .ssh/controller2.pem vagrant@controller2 echo "OK"
ssh -i .ssh/controller3.pem vagrant@controller3 echo "OK"
...
# Extra ceph.conf configuration to work around kolla-ansible bug https://bugs.launchpad.net/kolla-ansible/+bug/1763356
